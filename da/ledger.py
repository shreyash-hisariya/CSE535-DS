...
class Ledger:
    def __init__(self, pending_ledger_states,persistent_ledger_states,blockid_ledger_map,persistent_ledger_file,committed_block_map,persistent_ledger_entry,validator_info=None):
        ...
        self.persistent_ledger_entry = persistent_ledger_entry
        self.clear_file()
    ...
    def writeToFile(self,key,value):
        ...
        for t in value[1]:
            if t not in self.persistent_ledger_entry:
                final_transaction.append(t)
                self.persistent_ledger_entry.add(t)

        f = open(self.persistent_ledger_file, "a")
        msg="\nNew Commit: "+self.validator_info["Main"]["u"]+" ledger_state_id: " + str(key)+" block_id: "+str(value[0])+" Transaction: "+str(final_transaction)
        f.write(msg)
        f.close()
