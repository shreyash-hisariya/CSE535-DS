...
class Safety:
    ...
    def commit_state_id_candidate(self,block_round, qc):
        if qc is None:
            ...
            return self.validator_info["Ledger"].pending_state(qc.id)
        else:
            return None
    ...
    def make_vote(self,b, last_tc):
        ...
        if self.valid_signatures(b, last_tc) and self.safe_to_vote(b.round, self.qc_round, last_tc):
            ...
            if b.qc is None:
                vote_info = VoteInfo(b.id, b.round, -1, self.qc_round, self.validator_info["Ledger"].pending_state(b.id))
                hash_vote_info = self.hash_func(b.id, b.round, -1, self.qc_round,self.validator_info["Ledger"].pending_state(b.id))
            else:
                vote_info = VoteInfo(b.id, b.round, b.qc.vote_info.id, self.qc_round, self.validator_info["Ledger"].pending_state(b.id))
                hash_vote_info = self.hash_func(b.id, b.round, b.qc.vote_info.id, self.qc_round,self.validator_info["Ledger"].pending_state(b.id))
            #print("hello", vote_info.round)
            # TO DO hashing part.

            #print("CHECKKKKKKKKK",self.validator_info["Main"]["u"], " hash(vote_info):",hash_vote_info)
            if b.qc is not None: #need to verify this
                #print("*****1111**************** b.qc is not none",self.commit_state_id_candidate(b.round, b.qc))
                ledger_commit_info = LedgerCommitInfo(self.commit_state_id_candidate(b.round, b.qc), hash_vote_info) #need to verify for -1
            else:
                #print("*****2222****************b.qc is  none",self.commit_state_id_candidate(b.round, None))
                ledger_commit_info = LedgerCommitInfo(self.commit_state_id_candidate(b.round, None), hash_vote_info)
                #print("*****3333****************b.qc is  none", ledger_commit_info.commit_state_id)
            ...
            return VoteMsg(vote_info, ledger_commit_info, self.validator_info["BlockTree"].high_commit_qc,self.validator_info["Main"]["u"],signature)
        return None
